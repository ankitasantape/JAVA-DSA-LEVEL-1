...
public class Sudoko_Using_Bit_Manipulation {
	...
	public static void solveSudoku(int[][] arr, int[] rows, int[] cols, int[][] grid, int i, int j) {
		...
		if(arr[i][j] > 0) {
//			if col at their extreme pos then row reset to next row And 
//			if col at their extreme pos then col will reset and row already reset  
			...
			for(int num = 1; num <= 9; num++) {
				if(( rows[i] & (1 << num) ) == 0  && ( cols[j] & (1 << num) ) == 0  && ( grid[i / 3][j / 3] & (1 << num) ) == 0 ) {
					arr[i][j] = num;
					rows[i] ^= (1 << num); // bit ko toggle kr de means bit ko off se on kr denge
					cols[j] ^= (1 << num);
					grid[i / 3][j / 3] ^= (1 << num); 
					solveSudoku(arr, rows, cols, grid, j == arr[0].length - 1 ? i + 1: i, j == arr[0].length - 1 ? 0 : j + 1);
					grid[i / 3][j / 3] ^= (1 << num); 
					cols[j] ^= (1 << num); // bit ko toggle kr de means bit ko on se off kr denge
					rows[i] ^= (1 << num);
					arr[i][j] = 0;
				}
			}	
