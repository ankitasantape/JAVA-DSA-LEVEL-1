...
public class A33_All_Palindromic_Permutations {
   
	public static void generatepw(int cs, int ts, HashMap<Character, Integer> fmap, Character oddc, String asf) {
		if (cs > ts) { // aise krte krte agar apka current spot total spot ke beyond nikal gya hai
						// maltab sare spots apke ban gye hai to apki permutations tayaf hai asf ke
						// andar, uske sath permutations ka reverse bhi lagana hai
...
//			}
			...
			res += sb; // aur last mai result ke andar reverse add karo
		...
		for (char ch : fmap.keySet()) { // frquency map pe lagaya loop
			...
			if (freq > 0) { // jab apki frquency grater than 0 ho tabhi kuch kr skte ho
				...
				generatepw(cs + 1, ts, fmap, oddc, asf + ch); // agle spot keliye call laga diya. jo character nikala
																// tha use asf mai dal diya
	...
	public static void main(String[] args) {
		...
		// isme check karenge kitne character hai odd frequency wale, agar odd frequency
		// wala 1 se jada character nikla to hum kuch nhi kr skte iski palindromic
		// permutation possible hi nhi hai
		...
		generatepw(1, len, fmap, odd, ""); // shuruwat mai current spot 1 hai 1st character solve kr rhe hai, aur total
											// spots length tak solve karna hai. aur asf mai "".
	    scn.close();
	}
}
